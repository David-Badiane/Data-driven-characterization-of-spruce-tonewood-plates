function [Dataset_FA] = comsolRoutineFA_wedge(model, nSim, nModes, referenceVals,...
...
% - - - - - - - - - - - - - - - - - - - - - - - - - SETUP
    cd(csvPath);
    Dataset_FA = struct('inputs',[] ,'outputsEig',[] ,'outputsAmp',[] , 'nModes', []);
    Dataset_FA.nModes = nModes;
    ...
    if writeNow
        ...
        Dataset_FA.nModes = readMatrix('nModes.csv');
    end
    ...
    % set varNames for modeshapes csv files
    ...
    varNamesVel = {}; count = 1;
    % set varNames for outputsAmp csv file
    for ii = 1:length(velFilenames)
        for jj = 1:nModes
            varNamesVel{count} = [velFilenames{ii} '_f' num2str(jj)];
            count = count+1;
        end
    end
%- - - - - - - - - - - - - - - - - - - - - - - - - - - START SIMULATION LOOP
    ...
    for ii = start:nSim
        ...
        if ii == 1
            ...
            % beta uniform btw 2e-7 and 2e-4
            currentVals(12) = 2*10.^(rand(1,1)*3 - 7); 
        ...
        exportAllModesFromDataset(model, modesFileName,modeshapesFolder,expression);
        ...
        measPts = [];
        for jj = 1:length(velFilenames)
            measPts(jj) = str2num(velFilenames{jj}(end));
        end

        count = 1;
        for kk = measPts
            disp(['extracting amp of: cpt' int2str(kk) ' for ' velFilenames{count}]);
            exportData(model,['cpt' int2str(kk)], dirName,velFilenames{count},...
                'solid.u_tX*nx+ solid.u_tY*ny + solid.u_tZ*nz'); 
            [vel] = readTuples([velFilenames{count} '.txt'], 1, false);
            delete([velFilenames{count} '.txt']);
            vel = abs(vel(4:end));
            vels = [vels vel];
            count = count+1;
        end
        ...
        Dataset_FA.outputsAmp = [Dataset_FA.outputsAmp; vels];
        % Save results
        ...
        outputsAmpTable = writeMat2File(Dataset_FA.outputsAmp,'outputsAmp.csv', varNamesVel, length(varNamesVel),true);
        
        disp(['elapsed time for 1 tuple = ' , num2str(toc(tStart)), ' seconds']);
        disp(' '); 
