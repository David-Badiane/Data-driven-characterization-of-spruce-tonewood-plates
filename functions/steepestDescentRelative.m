...
fvals = []; % values of the function preallocation
    ...
    for k1 = 1:kmax1    %find the optimum step size(alpha) by line search
        ...
        disp(['step: ', num2str(alpha), ' x: ', num2str(x - alpha*g.*x),' fx0: ', num2str(fx0), ' fx1: ', num2str(fx1),' fx2: ', num2str(fx2),' fx: ', num2str(fx)] );
        ...
        if fx0 > fx1+TolFun && fx1 < fx2 - TolFun %fx0 > fx1 < fx2
            ...
            disp(['chosen step: ', num2str(alpha),' x:', num2str(x), ' fx0: ', num2str(fx0), ' fx: ', num2str(fx)] );                
