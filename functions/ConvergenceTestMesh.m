function [eigenFreqzMatrix, pointVelMatrix, eigRate, FDRate, timesEig, timesFD] = ConvergenceTestMesh(model,nModes)
...
% EIGENFREQUENCY STUDY
    ...
    for ii = 1:length(index)
        eigRate(ii) = mean((eigenFreqzMatrix(ii,:) - eigenFreqzMatrix(end,:))./eigenFreqzMatrix(end,:))*100;
    end
...
model.study('std2').feature('freq').set('plist', num2str(eigenFreqz(1:8).'));
    ...
    % relative error in percentage
    for ii = 1:length(index)
        FDRate(ii) = mean((pointVelMatrix(ii,:) - pointVelMatrix(end,:))./pointVelMatrix(end,:))*100;
    end
